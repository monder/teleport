...
func (g *GRPCServer) GenerateUserCertSet(ctx context.Context, req *proto.UserCertsRequest) (*proto.CertSet, error) {
	auth, err := g.authenticate(ctx)
	if err != nil {
		return nil, trail.ToGRPC(err)
	}
	certs, err := auth.AuthWithRoles.GenerateUserCertSet(req.Key, req.Username, req.Ttl, req.Compatibility)
	if err != nil {
		return nil, trail.ToGRPC(err)
	}
	return certs, err
